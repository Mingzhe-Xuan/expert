...
from ..backbones import EQUIFORMER_SO3_LAYOUT, EquiformerV2BackboneAdapter
...
FLOAT32_EQUIVARIANCE_TOLERANCE = 2.0e-3
...
def _silicon_graph(device: str):
    ...
    return build_periodic_graph(positions, cell, torch.tensor([14, 14], device=device), 12.0)


def run(device: str) -> dict[str, object]:
    torch.manual_seed(20260911)
    graph = _silicon_graph(device)
    adapter = EquiformerV2BackboneAdapter(SMOKE_LAYOUT, device=device)
    adapter.train()
    ...
    for name, rotation in (
        ...
        if error >= FLOAT32_EQUIVARIANCE_TOLERANCE:
            raise AssertionError(
                f"EquiformerV2 {name} equivariance error {error} exceeds tolerance"
            )
    ...
    if frozen_gradients:
        raise AssertionError("frozen EquiformerV2 checkpoint received gradients")
    if not interface_gradients or any(
        gradient is None or not torch.isfinite(gradient).all() for gradient in interface_gradients
    ):
        raise AssertionError("EquiformerV2 interface gradients are missing or non-finite")
    ...
    return {
        ...
        "runtime": f"fairchem-core=={importlib.metadata.version('fairchem-core')}",
        "source_layout_dimension": EQUIFORMER_SO3_LAYOUT.dimension,
