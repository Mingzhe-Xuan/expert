...
from ..experts import PointGroupTensorModel
    ...
    for index, sample in enumerate(samples, start=1):
        ...
        output.append(
            FrozenFeatureExample(
                ...
                target_cartesian=sample.target_cartesian.detach().to(torch.float32).unsqueeze(0),
            )
        )
...
def train_cached_backbone_readout(
    *,
    ...
    """Train a direct readout over a once-materialized frozen backbone tap."""

    if unit.dataset != "jarvis_tensor" or unit.target not in PUBLIC_TARGETS:
        raise ValueError("this benchmark runner is restricted to JARVIS dielectric/elastic")
    ...
    return {
        ...
        "public_targets": PUBLIC_TARGETS[unit.target],
        "exceeds_public_target": benchmark_target_comparison(unit.target, test_metrics),
        "history": history,
